    ...
    <section className={`w-full bg-[#FFF] flex items-center flex-col text-[#0A0B11] box-border ${isMobile ? 'mb-[40px] h-auto' : 'mb-[170px] min-h-screen '}`}>
      <div className={`w-full max-w-[1920px] mx-auto flex flex-col items-center relative box-border ${isMobile ? 'px-[50px]' : ''}`}>
        <div className={`font-[700] text-center max-w-[800px] leading-[1.2] relative ${isMobile ? 'text-[24px]' : 'text-[clamp(32px,3.5vw,42px)]'}`}>
          <img className={`h-auto object-contain absolute top-[-40%] ${isMobile ? 'w-[30px] right-[-8%]' : 'w-[52px] right-[-20%]'}`} src={assetPath('/images/black-red-icon.png')} alt="" />
          {t('oversee.title')}
        </div>
        <div className={`mt-[10px] text-center ${isMobile ? 'text-[14px]' : 'text-[clamp(16px,1.5vw,20px)]'}`}>{t('oversee.subtitle')}</div>
      </div>
      ...
      {isMobile ? (
        <div className='w-full mt-[clamp(30px,5vh,40px)] box-border'>
          <img 
            ...
            src={assetPath('/images/travel-oversee.png')} 
            alt="" 
          />
        </div>
      ) : (
        <div className='w-full mt-[clamp(30px,5vh,40px)] min-h-[800px] pl-[clamp(40px,16%,310px)] pr-[105px] py-[clamp(40px,8vh,80px)] flex justify-between gap-[clamp(20px,4vw,40px)] box-border bg-cover bg-no-repeat bg-center' style={{ backgroundImage: `url(${assetPath('/images/travel-oversee-bg.png')})` }}>
          <div className='w-[50%] flex flex-col justify-between'>
            <img 
              ...
              src={assetPath('/images/travel-happen-th.png')} 
              alt="" 
            />
            <img 
              className='max-w-[400px] w-[70%] rounded-[22px]'
              src={assetPath('/images/travel-happen-fo.png')} 
              alt="" 
            />
          </div>
