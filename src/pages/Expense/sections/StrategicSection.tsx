...
const StrategicSection = () => {
  ...
  const cards = t('strategic.cards', { returnObjects: true }) as Array<{
    title: string;
    description: string;
    category?: string;
    period?: string;
      ...
      <div className={`w-full max-w-[1920px] box-border ${isMobile ? 'flex flex-col mt-[50px] px-[50px] gap-[40px]' : 'flex flex-row flex-1 h-full mt-[6%] px-[clamp(40px,16%,310px)] gap-[30px]'}`}>
        <div className={`box-border rounded-[22px] bg-[#FFF] shadow-[0px_30px_70px_0px_rgba(0,0,0,0.10)] flex flex-col justify-between ${isMobile ? 'w-full p-[20px]' : 'w-[calc(100%/3-14px)] p-[clamp(16px,2vw,24px)]'}`}>
          <div className='flex flex-col gap-[20px] justify-between'>
            <div className={`${isMobile ? 'text-[20px]' : 'text-[clamp(20px,2.5vw,24px)]'}`}>{cards[0]?.title}</div>
            <div className={`opacity-60 ${isMobile ? 'text-[14px]' : 'text-[clamp(14px,1.5vw,16px)]'}`}>{cards[0]?.description}</div>
          </div>
          <div className={`font-[700] mt-[20px] ${isMobile ? 'text-[20px]' : 'text-[clamp(20px,2.5vw,24px)]'}`}>
            <Space className='flex-wrap'>
              <span className={`${isMobile ? 'text-[16px]' : 'text-[clamp(16px,1.8vw,18px)]'}`}>{cards[0]?.category}</span>
              <span className={`rounded-[8px] p-[8px] box-border bg-[#E6E7E7] ${isMobile ? 'text-[12px]' : 'text-[clamp(12px,1.4vw,14px)]'}`}>
                <span className='opacity-50 text-[#0A0B11]'>{cards[0]?.period}</span>
              </span>
            </Space>
          </div>
        ...
        <div className={`box-border rounded-[22px] bg-[#FFF] shadow-[0px_30px_70px_0px_rgba(0,0,0,0.10)] flex flex-col justify-between ${isMobile ? 'w-full p-[20px]' : 'w-[calc(100%/3-14px)] p-[clamp(16px,2vw,24px)]'}`}>
          <div className='flex flex-col gap-[20px] justify-between'>
            <div className={`${isMobile ? 'text-[20px]' : 'text-[clamp(20px,2.5vw,24px)]'}`}>{cards[1]?.title}</div>
        ...
        <div className={`box-border rounded-[22px] bg-[#FFF] shadow-[0px_30px_70px_0px_rgba(0,0,0,0.10)] flex flex-col justify-between ${isMobile ? 'w-full p-[20px]' : 'w-[calc(100%/3-14px)] p-[clamp(16px,2vw,24px)]'}`}>
          <div className='flex flex-col gap-[20px] justify-between'>
            <div className={`${isMobile ? 'text-[20px]' : 'text-[clamp(20px,2.5vw,24px)]'}`}>{cards[2]?.title}</div>
            <div className={`opacity-60 ${isMobile ? 'text-[14px]' : 'text-[clamp(14px,1.5vw,16px)]'}`}>{cards[2]?.description}</div>
            <Divider className={`${isMobile ? 'my-[16px]' : 'my-[clamp(16px,2vw,24px)]'}`}></Divider>
          </div>
